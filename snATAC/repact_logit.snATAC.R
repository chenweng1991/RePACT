...
repact_logit.snATAC <- function(OBJ, LSIrange, pheno, pheno.pvalCut, ){
    require(plot3D)
    require(reshape2)
    require(ggplot)
    require(Seurat)
    ...
    Select_LSI.sig <- Select_LSI[which(Select_LSI[,"pval"]<pheno.pvalCut),]
    ...
    ggplot(LSI.withinfo)+aes(pseudo.index.balanced,fill=get(pheno))+geom_density()+scale_fill_manual(values=c("steelblue","red"))+theme_classic()+theme(legend.position="none")+ labs(title = cell)
    ...
    print(p)
    # apply(tmpT2D.diffPeaks.20bin.LSI$UPDN.toplot,2,function(x){scale(x)}) %>% melt() %>% ggplot()+aes(Var1,Var2,fill=value)+geom_tile()+scale_fill_gradient2(low="steelblue",mid="white",high="red")+theme_classic()+theme(axis.text=element_blank())+ggtitle("LSI-20bins")+labs(x="T2D trajectory")
    saveRDS(tmpT2D.diffPeaks.20bin.LSI, file=paste("/mnt/rstor/genetics/JinLab/ssz20/zshanshan/differentiation_ATAC/processed/scATAC_Islet/",cell,".CARePACT.T2D_diffPeaks.20bin.LSI.rds",sep=''))
