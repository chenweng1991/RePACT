...
DonorWise.snATAC.RePACT <- function(OBJ, pheno){
    require(plyr)
    require(dplyr)
    require(Seurat)
    require(ggrepel)
    ...
    LSIInfo.20bin.ob.LSI <- CallT2Dpeak_qvalue(LSIInfo.20bin.ob$cellvsPeak.m.aggr,LSIInfo.20bin.ob$depths,LSIInfo.20bin.ob$index,qcut=0.01,slopecut1=0.5,slopecut2=-0.5,doscale=T)
    ...
    DN.hetero.peaks<-subset(FishersMethod.q.dn.df,qvalueInSample<0.01) %>% row.names
    DN.homo.peaks<-subset(FishersMethod.q.dn.df,qvalueInSample>=0.01) %>% row.names
    UP.hetero.peaks<-subset(FishersMethod.q.up.df,qvalueInSample<0.01) %>% row.names
    UP.homo.peaks<-subset(FishersMethod.q.up.df,qvalueInSample>=0.01) %>% row.names
    peakLis <- list(DN.interDonorPeak=DN.hetero.peaks, DN.intraDonorPeak=DN.homo.peaks, UP.interDonorPeak=UP.hetero.peaks, UP.intraDonorPeak=UP.homo.peaks)
    return(list(LSIInfo=LSIInfo, LSIInfo.20bin.ob.LSI=LSIInfo.20bin.ob.LSI, FishersMethod.q.df=FishersMethod.q.df, FishersMethod.q.dn.df=FishersMethod.q.dn.df, FishersMethod.q.up.df=FishersMethod.q.up.df, peakLis=peakLis))
}
