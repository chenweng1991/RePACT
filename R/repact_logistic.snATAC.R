...
#' @export
#' @examples
#' repact_logistic.snATAC(snATAC.OBJ, LSIrange=1:20, pheno="diseaseStat")

repact_logistic.snATAC <- function(OBJ, LSIrange, pheno){
    require(plot3D)
    require(reshape2)
    require(ggplot2)
    require(Seurat)
...
#    saveRDS(tmpT2D.diffPeaks.repact.bulk,paste(outputname,".T2D.diffPeaks.repact.bulk.rds",sep=''))
    return(list(LSI.withinfo=LSI.withinfo, Evenbin.donorContribute=Evenbin.donorContribute, tmpT2D.diffPeaks.20bin.LSI=tmpT2D.diffPeaks.20bin.LSI, tmpT2D.diffPeaks.repact.bulk=tmpT2D.diffPeaks.repact.bulk))
}
